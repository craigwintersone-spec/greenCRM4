// js/paperwork.js — Civara auto-fill paperwork (Phase 1: generic Start Form)
// ---------------------------------------------------------------------------
// What this does:
//   • Adds window.civaraGenerateStartForm() — opens a small confirm box,
//     prefilled from the open participant, and downloads a filled Word (.docx)
//     UKSPF Start Form.
//   • No database changes. No external libraries. It writes the .docx itself.
//
// How to switch it on (already wired if you took the new app.html):
//   1. Put this file at  js/paperwork.js
//   2. In app.html, near the other <script src="js/..."> lines, add:
//        <script src="js/paperwork.js"></script>
//   3. Add a button that calls it, e.g. in the Add Participant modal footer:
//        <button class="btn btn-ghost" onclick="civaraGenerateStartForm()">📄 Start Form</button>
//
// Later phases (per-contract uploaded templates + Evidence Hub) build on this.
'use strict';

(function () {

  /* ---------- tiny ZIP writer (store / no compression) ------------------ */
  // A .docx is just a ZIP of XML files. This makes a valid one with no deps.
  ...
  // value that may contain line breaks -> runs with <w:br/>
  ...
  function rowsTable(rows) {
    ...
    return '<w:tbl><w:tblPr><w:tblW w:w="9000" w:type="dxa"/>' + border +
  ...
  function buildStartFormXml(d) {
    ...
    const provider = [['Provider name', d.provider], ['Project name', d.project]];

    const body =
      para('UK Shared Prosperity Fund (UKSPF) Start Form', { center: true, bold: true, sz: 30, after: 40 }) +
      para('Participant start record — generated by Civara on ' + d.generatedOn, { center: true, sz: 18, color: '808080', after: 160 }) +
      heading('Part 1: Participant Details') + rowsTable(detail) +
      heading('Part 2: Participant Characteristics') + rowsTable(chars) +
      heading('Provider') + rowsTable(provider) +
      para('', { after: 120 }) +
  ...
  // Build a prefill object from (a) an optional participant object and
  // (b) the open Add Participant modal — tolerant to different field names.
  ...
  function openStartFormModal(d) {
    ...
    wrap.innerHTML =
      '<div style="background:#fff;border:1px solid #E0DAD0;border-radius:14px;max-width:640px;width:100%;margin:auto;padding:24px;box-shadow:0 8px 28px -12px rgba(23,86,85,.4);font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif">' +
      '<div style="font-size:20px;font-weight:700;color:#175655;margin-bottom:4px">Generate Start Form</div>' +
      '<div style="font-size:13px;color:#7A847F;margin-bottom:18px">Check the details below, complete anything missing, then download the filled Word form.</div>' +
      '<div style="display:flex;gap:14px;margin-bottom:14px;flex-wrap:wrap">' +
        '<div style="flex:1;min-width:220px">' + field('Provider name', 'pf-provider', d.provider) + '</div>' +
        '<div style="flex:1;min-width:220px">' + field('Project name', 'pf-project', d.project) + '</div>' +
      '</div>' +
      '<div style="font-size:12px;font-weight:700;color:#7A847F;text-transform:uppercase;letter-spacing:.5px;margin:6px 0 10px">Participant details</div>' +
      ...
      '</div>' +
      '<div style="font-size:12px;font-weight:700;color:#7A847F;text-transform:uppercase;letter-spacing:.5px;margin:10px 0 10px">Characteristics</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 14px">' +
        ...
        select('Basic skills (Maths & English)', 'pf-skills', d.basicSkills, ['', 'Yes', 'No']) +
  ...
  function doGenerate() {
    const d = {
      ...
      startDate: elVal('pf-start'), gender: elVal('pf-gender'), rightToWork: elVal('pf-rtw'),
      basicSkills: elVal('pf-skills'), labourStatus: elVal('pf-labour'), interpersonal: elVal('pf-inter'),
      generatedOn: new Date().toLocaleDateString('en-GB')
    };
    // remember provider/project so they don't retype every time
  ...
  /* ---------- public entry point ---------------------------------------- */
  // Call with a participant object, or with no argument to read the open
  // Add Participant modal. Safe to call from an onclick (ignores the event).
